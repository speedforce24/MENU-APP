...
const Admin = () => {
  ...
  // ✅ Stable average fetcher with guard
  useEffect(() => {
    ...
    api
      ...
      .then(async (res) => {
        if (!res.data || res.data.length === 0) {
          console.warn("⚠️ No foods found; keeping previous state.");
          return; // prevent wiping existing foods
        }
        ...
        const foodWithComments = await Promise.all(
          res.data.map(async (food) => {
            try {
              ...
              console.error(`Failed to fetch comments for ${food._id}`, err);
  ...
  // ✅ Submit handler with no state wipes
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // ✅ Keep old foods while adding new one
  ...
  const toggleFoodActive = async (foodId, currentStatus) => {
    ...
    try {
      ...
      setFoods((prev) =>
        prev.map((food) =>
          food._id === foodId ? { ...food, active: res.data.active } : food
        )
      );
    } catch (err) {
                    ...
                    <div className="flex items-center gap-4">
                      <label className="inline-flex relative items-center cursor-pointer">
                        <input
                          ...
                          onChange={() =>
                            toggleFoodActive(food._id, food.active)
                          }
                        />
